...
def data_augmentation(data: tf.Tensor) -> tf.Tensor:
	...
	if PIXEL_AUGMENTATION:
		...
		# Horizontal flips
		if not FLIP_DATASET:
			rand_tensor = tf.repeat(tf.random.uniform((batch_size,), minval = 0.0, maxval = 1.0, dtype = tf.float32), IMAGE_SIZE * IMAGE_SIZE * NB_CHANNELS)
		...
		rand_tensor = tf.random.uniform((batch_size,), minval = 0.0, maxval = 1.0, dtype = tf.float32)
		angles = tf.where(rand_tensor < AUGMENTATION_PROBA, angles, tf.zeros_like(angles))
		data = tfa.image.rotate(data, tf.cast(angles, tf.float32) * (math.pi / 2.0), interpolation = 'nearest', fill_mode = 'nearest')

		# Integer translations
		translations = tf.random.uniform((batch_size, 2), minval = int(-0.125 * IMAGE_SIZE), maxval = int(0.125 * IMAGE_SIZE), dtype = tf.int32)
		rand_tensor = tf.reshape(tf.repeat(tf.random.uniform((batch_size,), minval = 0.0, maxval = 1.0, dtype = tf.float32), 2), (batch_size, 2))
	...
	if GEOMETRIC_AUGMENTATION:
		...
		# Isotropic scaling
		scales = tf.random.uniform((batch_size,), minval = 1.0 - 0.125, maxval = 1.0 + 0.125, dtype = tf.float32)
		rand_tensor = tf.random.uniform((batch_size,), minval = 0.0, maxval = 1.0, dtype = tf.float32)
		scales = tf.where(rand_tensor < AUGMENTATION_PROBA, scales, tf.ones_like(scales))
		zeros = tf.zeros((batch_size,), dtype = tf.float32)
		transform = [scales, zeros, ((1.0 - scales) * IMAGE_SIZE) / 2.0, zeros, scales, ((1.0 - scales) * IMAGE_SIZE) / 2.0, zeros, zeros]
		transform = tf.transpose(transform, [1, 0])
		data = tfa.image.transform(data, transform, interpolation = 'bilinear', fill_mode = 'reflect')

		# Arbitrary rotations
		angles = tf.random.uniform((batch_size,), minval = 0.0, maxval = math.pi * 2.0, dtype = tf.float32)
		rand_tensor = tf.random.uniform((batch_size,), minval = 0.0, maxval = 1.0, dtype = tf.float32)
		angles = tf.where(rand_tensor < AUGMENTATION_PROBA, angles, tf.zeros_like(angles))
		data = tfa.image.rotate(data, angles, interpolation = 'bilinear', fill_mode = 'reflect')

		# Anisotropic scaling
		x_scales = tf.random.uniform((batch_size,), minval = 1.0 - 0.125, maxval = 1.0 + 0.125, dtype = tf.float32)
		y_scales = tf.random.uniform((batch_size,), minval = 1.0 - 0.125, maxval = 1.0 + 0.125, dtype = tf.float32)
		rand_tensor = tf.random.uniform((batch_size,), minval = 0.0, maxval = 1.0, dtype = tf.float32)
		...
		zeros = tf.zeros((batch_size,), dtype = tf.float32)
		transform = [x_scales, zeros, ((1.0 - x_scales) * IMAGE_SIZE) / 2.0, zeros, y_scales, ((1.0 - y_scales) * IMAGE_SIZE) / 2.0, zeros, zeros]
		transform = tf.transpose(transform, [1, 0])
		data = tfa.image.transform(data, transform, interpolation = 'bilinear', fill_mode = 'reflect')
